...
import { SPECIALITY_PALETTE, getThemeFromArtistSpecialities } from "@/lib/speciality-theme";
...
function stylesForSpecialityCard(specialities: { name: string; color: string }[]) {
  ...
  const avatarBg = theme.background.startsWith("linear-gradient")
    ? theme.background
    : theme.accentColor;
    ...
    <main className="min-h-screen bg-stone-50">
      ...
      <div className="bg-gradient-to-br from-amber-900 via-amber-800 to-amber-700 text-white px-6 py-20 text-center">
        ...
        <p className="text-amber-200 text-lg max-w-2xl mx-auto mb-6">
          Discover creative talents across music, theatre, media, literature & applied arts.
        </p>
        <div className="flex flex-wrap justify-center gap-4 mt-6 sm:mt-8">
          <Link
            href="/"
            className="px-5 py-2.5 bg-white text-amber-900 font-semibold rounded-lg hover:bg-amber-50 transition-colors text-sm"
          >
            ← Back to portal
          </Link>
          <a
            href="#colour-theming"
            className="px-5 py-2.5 border border-white/40 text-white font-semibold rounded-lg hover:bg-white/10 transition-colors text-sm"
          >
            Explore USPs ↓
          </a>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        ...
        <nav className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 mb-16">
          ...
          <ol className="grid sm:grid-cols-2 gap-2 text-sm">
            {[
              ...
              ["#multiregion", "6. Multi-region extensibility"],
              ["#home-spotlight", "7. Home spotlight & featured artist"],
              ...
              <li key={href}>
                <a
                  href={href}
                  className="text-amber-700 hover:text-amber-900 hover:underline underline-offset-2 transition-colors"
                >
                  {label}
                </a>
              </li>
          ...
          <p className="mt-4 border-t border-stone-100 pt-4 text-xs text-stone-500">
            In the repository: <code className="text-stone-600">docs/README.md</code> indexes
            maintainer documentation and points to the screenshot checklist in{" "}
            <code className="text-stone-600">docs/screenshots/README.md</code>.
          </p>
        </nav>
        ...
        <Section
          id="colour-theming"
          title="1. Speciality-based colour theming"
        ...
        >
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-800">
            <strong>How it works:</strong> Each speciality (Vocal, Violin, Mridangam, Veena…) has a
            unique hex colour in the admin-managed palette (mirrored in{" "}
            <code className="text-amber-950">lib/speciality-theme.ts</code> for tests). Artists can
            pick up to <strong>three</strong> specialities on registration; each tag uses its stored
            colour, and the card header uses those same values (solid for one colour, diagonal{" "}
            <code>linear-gradient</code> when several distinct colours apply). Single-speciality
            artists get a solid colour header. All text colours are WCAG AA compliant (≥4.5:1
            contrast ratio - verified by property-based tests).
          </div>
        </Section>
        ...
        >
          <div className="grid sm:grid-cols-2 gap-4 mb-6">
            {[
              {
                lang: "Tamil",
                sample: "கர்நாடக இசை என் உயிர். சிறு வயதிலிருந்தே வீணை வாசிக்கக் கற்றுக்கொண்டேன்.",
                script: "ta",
              },
              {
                lang: "Arabic",
              ...
              {
                lang: "Kannada",
                sample: "ಕರ್ನಾಟಕ ಸಂಗೀತ ನನ್ನ ಜೀವನದ ಭಾಗ. ನಾನು ಚಿಕ್ಕಂದಿನಿಂದ ವೀಣೆ ಕಲಿತೆ.",
                script: "kn",
              },
              {
                lang: "Hindi",
                sample: "कर्नाटक संगीत मेरे जीवन का अभिन्न अंग है। मैंने बचपन से मृदंगम सीखा।",
                script: "hi",
              },
              {
                lang: "Mixed",
        ...
        <Section
          id="search"
          title="3. Structured artist search"
          subtitle="Logged-in artists can find people by name, speciality, and availability dates - no LLM required."
        >
          ...
          <div className="bg-green-50 border border-green-200 rounded-xl p-5 text-sm text-green-800">
            <strong>Design decision:</strong> We deliberately avoided LLM-based NLP search to
            preserve user trust and keep the platform fully self-contained. The typeahead speciality
            dropdown is populated dynamically from approved artists in the DB - only specialities
            with at least one active artist are shown. All filtering is server-side SQL with no
            external API calls.
          </div>
        </Section>
        ...
        <Section
          id="auth"
          title="4. Magic-link authentication"
        ...
        >
            ...
            {[
              {
                ...
                body: (
                  <p className="text-sm text-stone-600 leading-relaxed">
                    ...
                    and type the same email you used when you registered. We send a single-use
                    sign-in link to that inbox.
                  </p>
                ),
              },
              {
                ...
                body: (
                  <p className="text-sm text-stone-600 leading-relaxed">
                    ...
                    <strong>Spam</strong>, <strong>Promotions</strong>, or similar folders in your
                    mail app.
                  </p>
                ),
              },
              {
                ...
                body: (
                  <p className="text-sm text-stone-600 leading-relaxed">
                    Your browser opens a short confirmation screen. Press <strong>Continue</strong>{" "}
                    once to complete sign-in. Use <strong>Log out</strong> in the header or
                    dashboard when you finish on a shared device.
                  </p>
                ),
              },
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-sm text-amber-800">
            <strong>Security (technical):</strong> Magic-link tokens are stored as SHA-256 hashes;
            issuing a new link invalidates older unused ones. The email points at{" "}
            <code className="text-amber-950">/auth/verify?token=…</code>; the page loads on{" "}
            <strong>GET</strong> only and the browser submits a <strong>POST</strong> to consume the
            token so mail previews and prefetch cannot burn the link. Links expire after 72 hours.
            Successful sign-in sets a 30-day <strong>session cookie</strong> backed by an{" "}
            <strong>HS256 JWT</strong>; Edge middleware validates it without a database round-trip
            on every request. Logout uses{" "}
            <code className="text-amber-950">POST /api/auth/logout</code>. Admin access comes from
            the <code className="text-amber-950">Artist.isAdmin</code> database flag. While signed
            in, a line <strong>above the site footer</strong> can show your{" "}
            <strong>profile display name</strong>, session expiry, and for admins{" "}
        ...
        <Section
          id="pwa"
          title="5. PWA & mobile-first design"
        ...
        >
            ...
            {[
              {
                icon: "📱",
                title: "Installable",
                desc: "Web App Manifest with standalone display mode. Add to home screen on iOS and Android.",
              },
              {
                icon: "📶",
                title: "Offline-capable",
                desc: "Service Worker caches the app shell and previously loaded artist profiles. Offline indicator shown.",
              },
              {
                icon: "🔔",
                title: "Push notifications",
                desc: "VAPID-based Web Push when your operator enables browser notifications. Email vs push preferences live under /profile/notifications.",
              },
            ].map(({ icon, title, desc }) => (
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-sm text-amber-800">
            <strong>Targets:</strong> Lighthouse PWA ≥90 · Performance ≥85 · Accessibility ≥90 (all
            on mobile profile). All interactive elements have a minimum 44×44 CSS pixel touch
            target. Semantic HTML throughout.
          </div>
        </Section>
        ...
        <Section
          id="multiregion"
          title="6. Multi-region extensibility"
          subtitle="Deploy for any country by updating environment configuration - zero code changes."
        >
          <div className="mb-6 overflow-x-auto rounded-2xl border border-stone-200 bg-white p-6 font-mono text-sm shadow-sm">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 text-sm text-blue-800">
            Set the deployment&apos;s country code, display name, locale, optional city labels, and
            optional city suggestions in environment variables. The home page maps artists whose
            saved city can be resolved to a real place.
          </div>
        </Section>
        ...
        <Section
          id="home-spotlight"
          title="7. Home spotlight & featured artist"
        ...
        >
          <div className="grid gap-6 sm:grid-cols-2 mb-6">
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              ...
              <p className="text-sm text-stone-600 leading-relaxed">
                The featured slot prefers <strong>Vocal</strong> artists and falls back to the full
                directory when needed. Which artist appears advances deterministically by{" "}
                <strong>calendar day</strong> in the deployment country&apos;s timezone (derived
                from <code>DEPLOYMENT_REGION</code> unless <code>DEPLOYMENT_TIMEZONE</code> is set).
                Ops can pin a vocalist for a given day via the <code>DailyFeatured</code> table.
              </p>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <p className="font-semibold text-stone-800 mb-3">Profile photo</p>
              <p className="text-sm text-stone-600 leading-relaxed">
                The card loads each artist&apos;s <strong>public profile photo URL</strong> (the
                HTTPS link they saved on their profile), with a coloured initial if the image cannot
                load.
              </p>
            </div>
          </div>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-sm text-amber-900">
            ...
            <code className="text-amber-950">getDailyFeaturedArtistForHome()</code> in{" "}
        ...
        <Section
          id="admin"
          title="8. Admin moderation tools"
        ...
        >
            ...
            {[
              {
                ...
                desc: "Approve or reject applicants with review comments, filters, and clear status handling. Approved artists receive a magic-link login email automatically.",
              },
              {
                icon: "🎵",
                title: "Artist management",
                desc: "Edit any artist profile, suspend accounts with validation and audit context, and keep directory records tidy.",
              },
              {
                icon: "🚩",
                title: "Reported photo moderation",
                desc: "Review reported profiles in a dedicated queue, resolve reports in bulk, clear current images, and suspend repeat offenders.",
              },
              {
                icon: "🎨",
                title: "Speciality management",
                desc: "Add/edit/delete specialities and their colour palette. Delete guard prevents removing specialities in use.",
              },
            ].map(({ icon, title, desc }) => (
              <div
                key={title}
                className="bg-white rounded-xl border border-stone-200 p-5 flex gap-4"
              >
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-sm text-amber-800">
            <strong>Access:</strong> Admin routes (<code>/admin/*</code>) are protected by Edge
            middleware. Admin role is stored on the artist row via the <code>isAdmin</code> database
            flag.
            <span className="ml-1">
              In dev, use{" "}
        ...
        <Section
          id="analytics"
          title="9. Product analytics & privacy (PostHog)"
          ...
          <div className="grid gap-6 sm:grid-cols-2 mb-6">
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              ...
              <ul className="text-sm text-stone-600 leading-relaxed list-disc pl-5 space-y-2">
                <li>
                  Explicit product events plus <strong>manual</strong> page views and PostHog
                  pageleave events (autocapture stays off).
                </li>
                <li>
                  Browser traffic in production uses a <strong>same-origin</strong>{" "}
                  <code className="text-stone-800">/api/ph</code> proxy so ingest hosts are not
                  embedded in the client bundle.
                </li>
                <li>
                  Server routes can emit events with{" "}
                  <code className="text-stone-800">artistId</code> as the PostHog distinct id - not
                  email.
                </li>
                <li>
                  Post-login redirects add <code className="text-stone-800">?ph_identify=1</code>{" "}
                  once to the artist or admin dashboard URL so the client can{" "}
                  <code className="text-stone-800">posthog.identify(artistId, …)</code> and align
                  browser analytics with the same id (query param is stripped on the next
                  navigation; local <code className="text-stone-800">/api/dev/login</code> does the
                  same in non-production).
                </li>
              </ul>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              ...
              <ul className="text-sm text-stone-600 leading-relaxed list-disc pl-5 space-y-2">
                ...
                <li>
                  Do Not Track and the <code className="text-stone-800">ph_opt_out</code> cookie are
                  respected on the client.
                </li>
                <li>
                  Visitors can use{" "}
                  ...
                  (opt-in / opt-out) for the full disclosure.
                </li>
              </ul>
            </div>
          </div>
          <div className="rounded-xl border border-stone-200 bg-stone-100 p-5 text-sm text-stone-700">
            <strong>Maintainer docs:</strong> Repository specs under{" "}
            <code className="text-stone-900">.kiro/specs/posthog-analytics/</code> and the operator
            guide <code className="text-stone-900">.kiro/steering/posthog-admin-guide.md</code>. Env
            template: <code className="text-stone-900">env.example</code> (PostHog block).
          </div>
        </Section>
        ...
        <Section
          id="tech"
          title="10. Tech stack at a glance"
        ...
        >
            ...
            <table className="w-full text-sm">
              <thead className="bg-stone-50 border-b border-stone-200">
                <tr>
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-stone-500 uppercase tracking-wide hidden sm:table-cell">
                    Why
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-stone-100">
                {[
                  [
                    ...
                    "SSR/SSG, API routes, Edge middleware, PWA-friendly",
                  ],
                  [
                    "Hosting",
                    "Cloudflare Workers + OpenNext",
                    "Production app on Workers via @opennextjs/cloudflare; Wrangler CI",
                  ],
                  ...
                  [
                    "Database",
                    "PostgreSQL via Neon",
                    "Serverless, git-like branching, no connection exhaustion",
                  ],
                  ["ORM", "Prisma", "Type-safe DB access, migrations"],
                  [
                    "Auth",
                    "Custom magic-link (JWT + Resend)",
                    "No passwords; verify flow resists mail-client prefetch",
                  ],
                  [
                    "Media",
                    "HTTPS profile assets",
                    "Artists supply vetted HTTPS URLs for photos; optional storage adapters in code when operators wire them",
                  ],
                  [
                    "Cache/sessions",
                    "Cloudflare KV (optional)",
                    "Edge-native when enabled; colocated with Workers",
                  ],
                  ...
                  [
                    "Analytics",
                    "PostHog (optional)",
                    "Explicit events, manual pageviews + pageleave, /api/ph proxy, Session Replay env-gated",
                  ],
                  [
                    "Testing",
                    "Vitest + fast-check",
                    "Unit + property-based tests (28 correctness properties)",
                  ],
                ].map(([layer, tech, why]) => (
          ...
          <div className="bg-stone-800 rounded-2xl p-6 text-white">
            <h3 className="font-bold mb-4 text-sm uppercase tracking-widest text-stone-400">
