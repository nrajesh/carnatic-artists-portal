...
/**
 * Replaces Next.js default global error UI. The built-in version uses a GET
 * `<form>` with no fields for "Reload"; submitting that appends a bare `?`
 * to the URL (empty query string). We reload via `location.reload()` instead.
 *
 * @see next/dist/client/components/builtin/global-error.js
 */
const errorThemeCss = `
:root {
  --next-error-bg: #fff;
  ...
  return (
    <html id="__next_error__" lang="en">
      ...
      <body>
        ...
        {digest ? (
          <p style={styles.digestFooter}>ERROR {digest}</p>
        ) : null}
      </body>
    </html>
  );
